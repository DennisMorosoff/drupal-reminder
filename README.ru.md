...
- `/add` - Зарегистрировать текущую группу для уведомлений
- `/fetch` - Получить содержимое сайта
- `/check` - Проверить последнюю статью (в личке делает рассылку по всем чатам, в группе — только в текущий чат)
